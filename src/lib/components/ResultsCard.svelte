<script lang="ts">
  ...
  import { discountStore } from '../stores/discounts';
  ...
  let discounts: any[] = [];
  discountStore.subscribe(state => { discounts = state.discounts; });
  
  $: finalPrice = hasTargetPrice ? targetPrice : results.finalPrice;
  $: priceWithIGV = finalPrice * (1 + IVA_RATE);
  $: igvAmount = finalPrice * IVA_RATE;
  $: hasResults = hasTargetPrice ? (targetPrice > 0 && results.originalPrice > 0) : true;
  $: showFullResults = hasTargetPrice || results.originalPrice > 0;
  $: hasActiveDiscounts = discounts.some(d => d.isActive && d.percentage > 0);
  
  $: simulatedResult = hasActiveDiscounts && results.originalPrice === 0 ? calculateConsecutiveDiscounts(100, discounts) : null;
  $: effectiveDiscountDisplay = results.originalPrice > 0 ? formatPercentage(results.effectiveDiscount) : (simulatedResult ? `${simulatedResult.effectiveDiscount.toFixed(2)}%` : '0%');
  $: savingsDisplay = results.originalPrice > 0 ? formatCurrency(results.savings) : 'S/ 0.00';
</script>
    ...
    <div class="results-grid">
      ...
      {:else}
        ...
        <div class="result-tile highlight-success">
          ...
          <div class="value-group">
            <span class="result-value success-text">{effectiveDiscountDisplay}</span>
            <span class="sub-value">Ahorro: {savingsDisplay}</span>
          </div>
        </div>
...
<style>
  ...
  .results-title {
    font-size: 0.7rem; font-weight: 800; color: var(--theme-text);
  ...
  .sub-value { font-size: 0.75rem; font-weight: 600; opacity: 0.7; margin-top: 2px; }
  
  .result-label {
    font-size: 0.75rem; color: var(--theme-muted); font-weight: 700;
  ...
  .purple-text { color: #8b5cf6; font-size: 1rem; }
